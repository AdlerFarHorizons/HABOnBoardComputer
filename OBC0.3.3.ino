...
int timeSinceLastCameraRun,timeSinceLastStateLog;
  ...
  if (timeSinceLastCameraRun>300000) {
...
char logString[200];
//Serial.println("in logStateVariables");
sprintf(logString,"$STATE %ld %ld %2d %02d %5.2f %4d %5.2f %4d %5.2f %8.1f %ld %6.2f %6.2f %6.2f %ld %6.2f %6.2f %ld %5d %5d %5d %ld %6.3f %6.3f %6.3f\n", 
...
if (strncmp(S,"$FHIAC",6)==0) { // extract raw magnetometer data
 
  float *tmpAX,*tmpAY,*tmpAZ;
  
  //first get the variables pointers we will drop these into
  tmpAX=va_arg(ap,float*);
  tmpAY=va_arg(ap,float*);
  tmpAZ=va_arg(ap,float*);

  S=toksplit(S,',',t,20);
  S=toksplit(S,',',t,20);                    //ignore the first value
  S=toksplit(S,',',t,20); *tmpAX=atof(t);    //extract the X component
  S=toksplit(S,',',t,20); *tmpAY=atof(t);    //extract the Y component
  S=toksplit(S,',',t,20); *tmpAZ=atof(t);    //extract the Z component
